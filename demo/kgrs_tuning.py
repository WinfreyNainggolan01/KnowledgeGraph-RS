...
class Dataloader:
    ...
    def _convert_kg(self, lines):
        ...
        kg = []
        rel_dict = {}
        relation2id_path = os.path.join(os.path.dirname(__file__), 'relation2id.txt')
        
        for line in open(relation2id_path, encoding='utf8').readlines():
...
class KGRSTuning:
    ...
    def tune_hyperparameters_combined(self, param_grid: dict, train_pos: np.array, train_neg: np.array, 
                                    test_pos: np.array, test_neg: np.array, 
                                    user_pos_items: dict, user_train_pos_items: dict):
        """
        Tune hyperparameters using combined AUC and nDCG@5 score
        """
        ...
        for i, params in enumerate(param_combinations):
            ...
            try:
                kgrs_temp = KGRSTuning(
                    train_pos=train_pos.copy(),
                    train_neg=train_neg.copy(),
                    kg_lines=self.kg,
                    n_user=self.dataloader.n_user,
                    n_item=self.dataloader.n_item
                )
                
                kgrs_temp.emb_dim = params.get("emb_dim", kgrs_temp.emb_dim)
                kgrs_temp.margin = params.get("margin", kgrs_temp.margin)
                kgrs_temp.learning_rate = params.get("learning_rate", kgrs_temp.learning_rate)
                kgrs_temp.epoch_num = params.get("epoch_num", kgrs_temp.epoch_num)
                
                kgrs_temp.model = TransE(
                    ent_num=kgrs_temp.dataloader.ent_num,
                    rel_num=kgrs_temp.dataloader.rel_num,
                    dataloader=kgrs_temp.dataloader,
                    margin=kgrs_temp.margin,
                    dim=kgrs_temp.emb_dim,
                    l1=kgrs_temp.l1,
                    learning_rate=kgrs_temp.learning_rate,
                    weight_decay=kgrs_temp.weight_decay,
                    device_index=kgrs_temp.device_index,
                )
                ...
                print(f"AUC: {auc:.4f}, nDCG@5: {ndcg5:.4f}, Combined: {combined_score:.4f}")
        ...
        for i, result in enumerate(results[:5]):
            ...
            print(f"   AUC: {result['auc']:.4f}, nDCG@5: {result['ndcg5']:.4f}, Combined: {result['combined']:.4f}")
        
        print(f"\nBest Parameters: {best_params}")
        print(f"Best Combined Score: {best_combined_score:.4f}")
        
        return best_params, best_combined_score
