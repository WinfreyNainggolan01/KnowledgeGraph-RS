...
from demo.kgrs_tuning import KGRSTuning
...
def evaluate(best_params=None):
    ...
    kgrs = KGRSTuning(
        ...
        kg_lines=open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', 'kg.txt')), encoding='utf-8').readlines(),
        n_user=n_user,
        n_item=n_item
    )

    if 'emb_dim' in best_params:
        kgrs.emb_dim = best_params['emb_dim']
    if 'margin' in best_params:
        kgrs.margin = best_params['margin']
    if 'learning_rate' in best_params:
        kgrs.learning_rate = best_params['learning_rate']
    if 'epoch_num' in best_params:
        kgrs.epoch_num = best_params['epoch_num']

    from demo.kgrs_tuning import TransE
    kgrs.model = TransE(
        ent_num=kgrs.dataloader.ent_num,
        rel_num=kgrs.dataloader.rel_num,
        dataloader=kgrs.dataloader,
        margin=kgrs.margin,
        dim=kgrs.emb_dim,
        l1=kgrs.l1,
        learning_rate=kgrs.learning_rate,
        weight_decay=kgrs.weight_decay,
        device_index=kgrs.device_index,
    )
...
def tune_and_evaluate():
    ...
    param_grid = {
        "emb_dim": [16, 20, 24],
        "margin": [25, 30, 35],
        "learning_rate": [1.5e-3, 2e-3, 2.5e-3],
        "epoch_num": [30, 35, 40]
    }

    print("Starting hyperparameter tuning...")
    best_params, best_combined_score = kgrs.tune_hyperparameters_combined(
        ...
        user_train_pos_items=user_train_pos_items
    )
...
if __name__ == '__main__':
    ...
    print("Training started with hyperparameter evaluation...")
    ...
    # Testing checkpoint
    # best_params = {'emb_dim': 24, 'margin': 25, 'learning_rate': 0.0025, 'epoch_num': 35}
    ...
    print("\n--- Result (with optimized hyperparameters) ---")
